...
namespace NRIApp.Droid.CustomRenderers
{
    public static class ImageHelpers
    {
        ...
        {
            byte[] imageBytes;
            //imageBytes = System.Text.Encoding.UTF8.GetBytes(path);

             var originalImage = BitmapFactory.DecodeFile(path);
            //var originalImage = BitmapFactory.DecodeFileAsync(path);

            
            // var rotation = GetRotation(path);
            //var width = (originalImage.Width * scaleFactor);
            //var height = (originalImage.Height * scaleFactor);
            var scaledImage = Bitmap.CreateBitmap(originalImage);
            //CreateScaledBitmap(originalImage, (int)width, (int)height, true);
